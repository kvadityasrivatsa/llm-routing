...
class DataTrainingArguments:
    ...
    train_file: Optional[str] = field(
        default=None, metadata={"help": "The path of the training dataset file."}
    )
    test_file: Optional[str] = field(
        default=None, metadata={"help": "The path of the test dataset file."}
    )
    validation_file: Optional[str] = field(
        default=None, metadata={"help": "The path of the validation dataset file."}
    )
...
def main():
    ...
    # model = deleteEncodingLayers(org_model, [0, 1, 10, 11])
    ...
    def preprocess_function(examples):
        ...
        # Tokenize the texts
        result = tokenizer(examples["sentence"], padding=padding, max_length=max_seq_length, truncation=True, return_tensors="pt")
    ...
    #Initialize our weighted Trainer
    trainer = WeightedTrainer(
        ...
        class_weights=torch.FloatTensor([[1.71, 0.81, 1.23, 0.64, 1.19, 1.01]]),
    )
    ...
    if training_args.do_predict:
        ...
        input_sentences = predict_dataset["question"]
        input_splits = predict_dataset["split"]
        # Removing the `label` columns if exists because it might contains -1 and Trainer won't like that.
        if "label" in predict_dataset.features:
            predict_dataset = predict_dataset.remove_columns("label")
        raw_predictions = trainer.predict(predict_dataset, metric_key_prefix="predict").predictions
        
        if is_regression:
            predictions = np.squeeze(raw_predictionss)
        elif is_multi_label:
        ...
        if trainer.is_world_process_zero():
            ...
            for index, (item, raw_prediction, sentence, split, true_label) in enumerate(zip(predictions, raw_predictions, input_sentences, input_splits, true_labels)):
            #for index, item in enumerate(predictions):
                if is_regression:
                    writer.write(f"{index}\t{item:3.3f}\n")
                elif is_multi_label:
                    # recover from multi-hot encoding
                    item_lab = [label_list[i] for i in range(len(item)) if item[i] == 1]
                    true_label = [label_list[i] for i in range(len(true_label)) if true_label[i] == 1]
                    collect_predictions.append([sentence, split, true_label, item_lab, item, raw_prediction])
                else:
                    item_lab = label_list[item]
                    true_label = label_list[true_label]
                    collect_predictions.append([sentence, split, true_label, item_lab, item, raw_prediction])
        
        header = ["question", "split", "true_label", "prediction", "raw_predictions", "confidence"]
        collect_predictions_df = pd.DataFrame(collect_predictions, columns=header)
        output_predict_path = os.path.join(training_args.output_dir, "predict_results.csv")
        collect_predictions_df.to_csv(output_predict_path, index=False)                        
        logger.info("Predict results saved at {}".format(output_predict_path))
